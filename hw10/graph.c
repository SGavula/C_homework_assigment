...
void load_bin(const char *fname, graph_t *graph) {
    ...
    while(fread(&graph->edges[graph->num_edges], sizeof(edge_t), 1, file)) {
        graph->edges[graph->num_edges].from = ntohl(graph->edges[graph->num_edges].from); 
        graph->edges[graph->num_edges].to = ntohl(graph->edges[graph->num_edges].to); 
        graph->edges[graph->num_edges].cost = ntohl(graph->edges[graph->num_edges].cost); 
        graph->num_edges++;
        if(graph->num_edges >= graph->capacity){
            graph = enlarge_graph(graph);
        }
...
void save_bin(const graph_t * const graph, const char *fname) {
    ...
    for(int i = 0; i < graph->num_edges; i++) {
        ...
        edge.from = htonl(graph->edges[i].from);
        edge.to = htonl(graph->edges[i].to);
        edge.cost = htonl(graph->edges[i].cost);
