...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (pq && pq->len < pq->size && label >= 0 && label < pq->size && pq->heapIDX[label] != -1) {
      ...
      int parent_index = GET_PARENT(idx);

      while(pq->cost[parent_index] > pq->cost[pq->heapIDX[label]]) {
         pq_swap(pq, idx, parent_index);
         // int t;
         // pq->heapIDX[pq->label[idx]] = parent_index;
         // pq->heapIDX[pq->label[parent_index]] = idx;
         // t = pq->label[idx];
         // pq->label[idx] = pq->label[parent_index]; 
         // pq->label[parent_index] = t;
         // t = pq->cost[idx]; 
         // pq->cost[idx] = pq->cost[parent_index];
         // pq->cost[parent_index] = t;
         idx = pq->heapIDX[label];
         parent_index = GET_PARENT(idx);
         if(idx <= 0 || parent_index < 0 || idx >= pq->len || parent_index >= pq->len) {
            break;
         }
      }
